...
void	have_to_do_sa(t_node **head_stack_a)
{
	if (has_to_tri(head_stack_a) == 1 \
		&& (*head_stack_a)-> index > (*head_stack_a)-> next -> index)
	{
...
void	ft_tri_three(t_node **head_stack_a)
{
	...
	{
		if (has_to_tri(head_stack_a) == 1 \
			&& which_three_index_is_max(head_stack_a) == 1)
		{
			ra(head_stack_a, NULL);
			have_to_do_sa(head_stack_a);
		}
		else if (has_to_tri(head_stack_a) == 1 \
			&& which_three_index_is_max(head_stack_a) == 2)
		{
			rra(head_stack_a, NULL);
			have_to_do_sa(head_stack_a);
		}
		else if (has_to_tri(head_stack_a) == 1 \
			&& which_three_index_is_max(head_stack_a) == 0)
		{
			sa(head_stack_a, NULL);
		}
	}
